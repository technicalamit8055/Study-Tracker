...
class LocalCollection {
  ...
  matches(item, query) {
    for (const key of Object.keys(query)) {
      // Treat an explicit null in a query as "field absent or null" so that
      // legacy documents written before a field existed still match.
      if (query[key] === null) {
  ...
  async findOne(query) {
    ...
    return list.find(item => this.matches(item, query)) || null;
  }

  async find(query) {
    const data = readLocalStore();
    const list = data[this.name] || [];
    return list.filter(item => this.matches(item, query));
  }
  ...
  async updateOne(query, update, options = {}) {
    const data = readLocalStore();
    if (!data[this.name]) data[this.name] = [];
    let itemIndex = data[this.name].findIndex(item => this.matches(item, query));
...
async function connectToDatabase() {
  ...
  try {
    ...
    try {
      ...
      // Progress is keyed per (user, exam). Drop the legacy unique index on
      // userId alone, which would reject a user's second exam roadmap.
      try {
        const existing = await db.collection('progress').indexes();
        const legacy = existing.find(
          ix => ix.unique && ix.key && ix.key.userId === 1 && Object.keys(ix.key).length === 1
        );
        if (legacy) {
          await db.collection('progress').dropIndex(legacy.name);
        }
      } catch (dropErr) {}

      await db.collection('progress').createIndex({ userId: 1, examId: 1 }, { unique: true });
    } catch (idxErr) {}
